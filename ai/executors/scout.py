...
def find_corners(center, area):
    ...
    corners = [xy2hex(center_row, col_max_center),
...
def get_end_point(start, direc, dist):
    ...
    flag = row & 1
    delta_row, delta_col = (dist + 1) // 2 * directions[flag][direc] + \
...
class ScoutExecutor:
    ...
    def allocate_traj(self, start, center):
        ...
        for i in layers:
            j = i - first_row
            if flag == 0:
                total_traj.append(xy2hex(self.xy_points[j][1][0], self.xy_points[j][1][1]))
                total_traj.append(xy2hex(self.xy_points[j][-2][0],self.xy_points[j][-2][1]))
            else:
                total_traj.append(xy2hex(self.xy_points[j][-2][0],self.xy_points[j][-2][1]))
                total_traj.append(xy2hex(self.xy_points[j][1][0], self.xy_points[j][1][1]))
            flag = 1 - flag
    ...
    def execute(self, task, agent):
        ...
        for obj_id, unit in agent.valid_units.items():
            ...
            to_detected = list(self.suspected) if self.suspected else list(self.unscouted)
